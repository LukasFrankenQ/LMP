...
TEMP FILE, MAJOR CODE DUPLICATION WITH scripts/build_load_weights.py

**Outputs**

- ``RESOURCES/dno_load_weights.csv``: share of load attributatable to each DNO region
...
if __name__ == "__main__":
    ...
    dno_regions = gpd.read_file(snakemake.input["dno_regions"]).set_index('LongName')
    load_weights = pd.DataFrame(index=dno_regions.index, columns=["load_weight"])

    for i, row in dno_regions.iterrows():
    ...
    assert np.isclose(load_weights.sum(), 1.), "Sum of load weights is not 1."
    (
        pd.concat((dno_regions, load_weights), axis=1)
        [["load_weight"]]
        .fillna(0)
